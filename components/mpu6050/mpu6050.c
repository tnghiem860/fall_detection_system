...
static esp_err_t mpu6050_read_bytes(uint8_t reg_addr, uint8_t *data,
                                    size_t len) {
...
esp_err_t mpu6050_read(mpu6050_data_t *data) {
  ...
  data->accel_magnitude =
      sqrtf(data->ax_g * data->ax_g + data->ay_g * data->ay_g +
            data->az_g * data->az_g);
...
int mpu6050_read_fifo(mpu6050_data_t *samples, int max_samples) {
  ...
  uint8_t cnt_buf[2];
  if (mpu6050_read_bytes(MPU6050_REG_FIFO_COUNT_H, cnt_buf, 2) != ESP_OK)
    return 0;
  ...
  for (int i = 0; i < num_samples; i++) {
    uint8_t buf[6];
    if (mpu6050_read_bytes(MPU6050_REG_FIFO_R_W, buf, 6) != ESP_OK)
      break;
    ...
    samples[i].accel_magnitude = sqrtf(samples[i].ax_g * samples[i].ax_g +
